...
SOURCES_FILES	+=	cmd_utils.c pipex_utils.c cmd.c
...
runv:
				clear && $(MAKE) && cat "passwd" && echo > passwd2 && valgrind -q --leak-check=full --show-leak-kinds=all -s --track-origins=yes ./pipex "passwd" "tr a z" "tr z b" "tr b c" "tr c 1" "tr 1 y" "passwd2" && echo "->" && cat "passwd2" && echo "<-"

runvv:
				clear && $(MAKE) && cat "passwd" && echo > passwd2 && valgrind -q --leak-check=full --show-leak-kinds=all -s --track-origins=yes ./pipex "passwd" "tr a z" "tr z b" "passwd2" && echo "->" && cat "passwd2" && echo "<-"
